...
(defn step-one
  ...
  (let [initial (:position point)
        shift (map #(* steps %) (:velocity point))]
    (map + initial shift)))

(defn step
  [array steps]
  (map #(partial (step-one % steps)) array))

(defn point-dist
  [cord-pair]
  (let [ax (first (first cord-pair))
        ay (last (first cord-pair))
        bx (first (last cord-pair))
        by (last (last cord-pair))
        x-square (* (- ax bx) (- ax bx))
        y-square (* (- ay by) (- ay by))]
    (Math/sqrt (+ x-square y-square))))

(defn average-distance
  [points]
  (let [combinations (combo/combinations points 2)
        distances (map point-dist combinations)]
